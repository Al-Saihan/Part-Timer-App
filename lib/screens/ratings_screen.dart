...
class _RatingsScreenState extends State<RatingsScreen>
                  ...
                  builder: (context, snap) {
                    ...
                    return ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final jobTitle =
                            item['job_title'] ?? item['jobTitle'] ?? 'Job';
                        final other =
                            item['other_user'] ??
                            item['otherUser'] ??
                            item['other'] ??
                            {};
                        final name = other is Map
                          ...
                          child: ListTile(
                            ...
                            trailing: canRate
                                ? ElevatedButton(
                                    ...
                                    onPressed: () =>
                                        _showRateDialogForItem(item),
                                    child: Text(
                                      existing != null ? 'Edit' : 'Rate',
                  ...
                  builder: (context, snap) {
                    ...
                    if (items.isEmpty)
                      return ListView(
                        ...
                        children: const [
                          SizedBox(height: 120),
                          Center(child: Text('You have not rated anyone yet')),
                        ],
                      );
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            title: Text(
                              rated is Map ? (rated['name'] ?? 'User') : 'User',
                            ),
                            subtitle: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                const SizedBox(height: 4),
                                Text(
                                  'Job: ${job is Map ? (job['title'] ?? job['job_title'] ?? '') : ''}',
                                ),
                  ...
                  builder: (context, snap) {
                    ...
                    if (items.isEmpty)
                      return ListView(
                        ...
                        children: const [
                          SizedBox(height: 120),
                          Center(child: Text('No ratings about you yet')),
                        ],
                      );
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            title: Text(
                              rater is Map ? (rater['name'] ?? 'User') : 'User',
                            ),
                            subtitle: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
  ...
  void _showRateDialogForItem(Map<String, dynamic> item) {
    final other =
        item['other_user'] ?? item['otherUser'] ?? item['other'] ?? {};
    ...
    final existing = item['existing_rating'];
    if (existing is Map) {
      rating = (existing['rating'] is int)
          ? existing['rating']
          : int.tryParse(existing['rating']?.toString() ?? '') ?? rating;
      reviewCtrl.text =
          (existing['review'] ?? existing['comment'] ?? '')?.toString() ?? '';
    }
        ...
        return StatefulBuilder(
          builder: (ctx2, setState) {
            return AlertDialog(
              backgroundColor: const Color.fromARGB(255, 213, 240, 255),
              title: Text(
                'Rate ${other is Map ? (other['name'] ?? 'User') : 'User'}',
              ),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  ...
                  onPressed: loading
                      ...
                      : () async {
                          if (ratedId == null || jobId == null) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text('Missing user/job id'),
                              ),
                            );
                            return;
                          }
